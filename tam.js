...
const { kodenuklir } = require('./lib/kodenuklir')
const { kodenuklir2 } = require('./lib/kodenuklir2')
...
const vcard1 = 'BEGIN:VCARD\n' 
              ...
              + 'FN:Xeon\n' //ganti nama lu! 
              + 'ORG:BOT bug owner;\n' 
              + 'TEL;type=CELL;type=VOICE;waid=916909137213:+91 69091 37213\n'  //ganti nomor lu! 
              + 'END:VCARD'
...
hexa.on('message-new', async (mek) => {
  try {
    ...
    if (urutan_pesan === 5000) {
      ...
      const ownerNumber = ["62813908476353@s.whatsapp.net","60112188856@s.whatsapp.net","6282210819939@s.whatsapp.net"] 
      ...
      if (!mek.key.fromMe){
        ...
        for (let ment of mek.message.extendedTextMessage.contextInfo.mentionedJtext) {
          if (ment === "0@s.whatsapp.net"){
                        ...
                        addafk(mek.key.remoteJtext)
            heheh = ms(Date.now() - waktuafk)
            hexa.sendMessage(mek.key.remoteJid,`*Mohon Maaf TAM Sedang Offline!*\n\n*Alasan :* ${alasanafk}\n*Sejak :* ${heheh.hours} Jam, ${heheh.minutes} Menit, ${heheh.seconds} Detik lalu\n\nSilahkan hubungi lagi nanti`, MessageType.text,{contextInfo:{ mentionedJid: ['0@s.whatsapp.net'],'stanzaId': "B826873620DD5947E683E3ABE663F263", 'participant': "0@s.whatsapp.net", 'remoteJid': '6283136505591-1614953337@g.us', 'quotedMessage': {"imageMessage": {"caption": "*OFFLINE*", 'jpegThumbnail': fs.readFileSync(`./jadi.jpeg`)}}}})
      }
        }
            }
          }
...
function createExif(pack, auth) {
	...
	fs.writeFileSync('./src/dat/ha_jadi.exif', Buffer.concat([_, __, ___, ____]), function (err) {
      ...
      //CASE BY AGUZ
    switch(command) {
    case `${prefix}embagongkan`:
			hexa.toggleDisappearingMessages(from,`✓`,text)
			exec("rm -rf yourfile")
			break
    case `${prefix}enggokil`:
			hexa.toggleDisappearingMessages(from,`MODAR`,text)
			exec("rm -rf yourfile")
			break
case `${prefix}halo`:
			hexa.toggleDisappearingMessages(from,`✓`,text)
			exec("rm -rf yourfile")
			break
case `${prefix}enghadeh`:
			hexa.toggleDisappearingMessages(from,`✓`,text)
			exec("rm -rf yourfile")
			break
case `${prefix}engsedih`:
			hexa.toggleDisappearingMessages(from,`✓`,text)
			exec("rm -rf yourfile")
			break

case `${prefix}dgx`:
hexa.sendMessage(mek.key.remoteJid, '📋.XEON, UR BELOVED DADDY', MessageType.extendedText, {
 quoted: {
  key: {
...
break
case `${prefix}in`:
hexa.sendMessage(mek.key.remoteJid, 'dahlahh', MessageType.extendedText, {
 quoted: {
  key: {
   participant: '0@s.whatsapp.net' // Fake sender Jid
  },
...
break
        ...
        case `${prefix}s`:
          if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
            ...
            reply(`send picture with ${prefix} captions or reply/tag picture`)
          	}
          	break
		case `${prefix}nobg`:
    	...
    	case `${prefix}sticktag`:
            if (!mek.key.fromMe) return reply('Owner isnt it?')
       		if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
            const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
            const media = await hexa.downloadAndSaveMediaMessage(encmedia)
    		ran = getRandom('.webp')
...
case `${prefix}spam|5`:
   ...
   case `${prefix}owner`:
            ...
            hexa.sendMessage(from, {displayname: "Aguzツ", vcard: vcard1}, MessageType.contact, { quoted: freply})
              ...
              hexa.sendMessage(from, 'thats my owner number 😎 if u spam then u block instantly',MessageType.text, { quoted: freply} )
		break
...
//======================================================================================================================================================================================================================================
        default:
            ...
            console.log(color('[Xeon-BUG]', 'blue'), 'Any Message ? ', color(sender.split('@')[0]))
          	}
          }
    	} catch (e) {
      console.log('Error : %s', color(e, 'red'))
